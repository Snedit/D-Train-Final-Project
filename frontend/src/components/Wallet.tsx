...
import { Wallet as WalletIcon, CreditCard, History, TrendingUp, CheckCircle, XCircle, X } from 'lucide-react';
...
const Wallet: React.FC<WalletProps> = ({ onBack }) => {
  ...
  const verifyStripeSession = async (sessionId: string) => {
    try {
      ...
      if (data.newBalance !== undefined && data.newBalance !== null) {
        ...
        setSuccess('Wallet recharged successfully via Stripe!');
        fetchWalletData();
  ...
  // Since we're modal-based the return_url won't actually navigate away —
  // but we still handle the URL params if the user somehow lands here directly.
  useEffect(() => {
  ...
  // Fetch clientSecret from backend — called lazily when user clicks Pay
  ...
  const handleRecharge = async () => {
    ...
    setSuccess('');
    try {
      // Pre-fetch the secret so the modal opens immediately
  ...
  // Called when Stripe embedded checkout fires onComplete
  ...
  const handleDevTopup = async () => {
    ...
    try {
      const token = localStorage.getItem('dtrain_token');
      const res = await fetch(`${API_BASE}/wallet/fake-topup`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
        body: JSON.stringify({ amount: rechargeAmount }),
      });
      ...
      if (data.newBalance !== undefined) {
        setBalance(data.newBalance);
        setSuccess(`[DEV] Added ₹${rechargeAmount} to wallet.`);
        fetchWalletData();
      } else {
        setError(data.message || 'Dev top-up failed');
      }
    } catch (err: any) {
      setError(err.message || 'Dev top-up failed');
    } finally {
      setProcessing(false);
    }
  };
        ...
        {checkoutOpen && clientSecret && (
          ...
          >
            <motion.div
              ...
              className="relative w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-[24px] border-[3px] border-slate-900 bg-white shadow-[12px_12px_0_0_rgba(15,23,42,1)]"
            >
              {/* Modal Header */}
            ...
            <div className="relative z-10 px-6 py-7 md:px-10 md:py-9">
              ...
              <nav className="flex items-center justify-between mb-8">
                ...
                <motion.button
                  ...
                  onClick={onBack}
                  className="flex items-center px-6 py-2 rounded-[12px] border-[3px] border-slate-900 bg-blue-400 text-white text-sm font-semibold shadow-[4px_4px_0_0_rgba(15,23,42,1)] transition-all hover:-translate-y-0.5 hover:bg-blue-500"
                >
                  Back to Dashboard
                </motion.button>
              </nav>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
                ...
                <div className="lg:col-span-2 rounded-[22px] border-[3px] border-slate-900 bg-white p-8 shadow-[8px_8px_0_0_rgba(15,23,42,1)]">
                  ...
                  {/* Stripe Pay Button — opens modal, no redirect */}
                  <motion.button
                    whileHover={{ y: -2 }} whileTap={{ y: 0 }}
                  ...
                  <motion.button
                    ...
                    disabled={processing}
                    className="w-full mt-3 px-6 py-3 rounded-[12px] border-[2px] border-dashed border-slate-400 text-slate-500 text-sm font-bold transition-all hover:border-slate-600 hover:text-slate-700 disabled:opacity-40"
                  >
                    ⚙️ Dev Mode — Add Fake Balance
                  </motion.button>
                </div>
              </div>
                          ...
                          >
                            ...
                            <td className="px-6 py-4">
                              <span className="text-lg font-extrabold text-slate-900">
                                {tx.type === 'charge' || tx.type === 'withdrawal' || tx.type === 'reservation'
                                  ? `-₹${tx.amount.toFixed(2)}`
                                  : `+₹${tx.amount.toFixed(2)}`}
                              </span>
                            </td>
