...
ipcMain.handle("get-device-id", async () => {
  ...
  try {
    if (fs.existsSync(configPath)) {
      ...
      if (config.deviceId) {
        console.log(
          `${icons.mobile} Loaded deviceId from disk:`,
          config.deviceId,
        );
...
ipcMain.handle("run-test-job", async (event, jobId, passedDeviceId) => {
  ...
  if (!deviceId) {
    throw new Error(
      `${icons.error} No deviceId available. Please register first.`,
    );
  }
  ...
  console.log(
    `${icons.rocket} Starting job ${jobId} (${shortId}) for worker ${deviceId}`,
  );
  ...
  try {
    ...
    await sendLog(
      `${icons.clipboard} Main file: ${jobData.config?.entryFile || "main.py"}\n`,
    );
    ...
    await sendLog(
      `${icons.check} ZIP downloaded (${(zipBuffer.length / 1024 / 1024).toFixed(1)}MB)\n`,
    );
    ...
    if (!fs.existsSync(mainFilePath)) {
      throw new Error(
        `${icons.error} Main file '${mainFileName}' not found in ZIP`,
      );
    }
...
CMD ["python", "${mainFileName}"]
      ...
      const sendDockerStats = () => {
        exec(
          ...
          (err, stdout) => {
            ...
            console.log(
              `[DockerStats] cpu=${cpu}% mem=${memory}% | sending to renderer`,
            );
            if (mainWindow && !mainWindow.isDestroyed()) {
              ...
              console.warn(
                `[DockerStats] mainWindow not available — metrics dropped`,
              );
            }
          },
        );
    ...
    await new Promise((resolve) => {
      exec(
        ...
        async (error, stdout, stderr) => {
          if (error)
            await sendLog(`${icons.warning} Copy warning: ${stderr}\n`);
          resolve();
        },
      );
    });
    ...
    const outputFiles = files.filter((f) => {
      ...
      return (
        stats.isFile() &&
        !["main.py", "requirements.txt", "Dockerfile", "__pycache__"].includes(
          f,
        ) &&
        !f.startsWith(".")
      );
    ...
    await sendLog(
      `${icons.check} Output ZIP created: ${(zipStats.size / 1024 / 1024).toFixed(2)} MB\n`,
    );
    ...
    const uploadResponse = await fetch(
      `${API_BASE}/api/jobs/${jobId}/complete`,
      {
        method: "POST",
        body: formData,
      },
...
ipcMain.handle("launch-docker", async () => {
  return new Promise((resolve) => {
    if (process.platform === "win32") {
      ...
      const dockerPath = possiblePaths.find((p) => fs.existsSync(p));
      if (!dockerPath) return resolve({ success: false, installed: false });
      exec(`"${dockerPath}"`, (err) =>
        resolve({ success: !err, installed: true }),
      );
    } else if (process.platform === "darwin") {
      exec(`open "/Applications/Docker.app"`, (err) =>
        resolve({ success: !err, installed: true }),
      );
    } else {
      exec("systemctl start docker", (err) =>
        resolve({ success: !err, installed: true }),
      );
    }
  });
